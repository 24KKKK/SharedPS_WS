...
public class ParklotUtil {
	...
	public static String getParklotId(String parklotName) {
		SysoUtils.print("传入的停车场名称为为：" + parklotName);
		String id = null;
		DBBean dbBean = new DBBean();
		String sql = "select parklotAdminId  from table_parklotInfo where parklotName = " + "'" + parklotName + "'";
		SysoUtils.print("查询id的sql为：" + sql);
		ResultSet rSet = dbBean.executeQuery(sql);
		try {
			while (rSet.next()) {
				id = rSet.getString("parklotAdminId");
			}
		...
		SysoUtils.print("查询到的停车场管理员id为：" + id);
		return id;
	}
